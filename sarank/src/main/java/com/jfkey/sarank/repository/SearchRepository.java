...
//			+ "RETURN  title, paID, authors, authorsID, year, venue,jouID, conID, citations, versions, doi, score; ")
	...
	@Query("WITH {paIDs} AS coll UNWIND coll AS col MATCH (p:Paper)-[:PaPAA]->(r)-[:PAAAth]-(a:Author)  WHERE p.paID = col WITH DISTINCT ( p.paID ) AS paID , a.athID AS authorsID, a.athName AS authors, p.originalTitle AS title, p.NormalizedName AS venue,  p.paYear AS year, p.jouID AS jouID, p.conID AS conID, r.authorNumber AS number, p.cts AS citations, p.paDOI AS doi, p.paScore AS saRank, p.fr as futureRank, p.pr as pageRank ORDER BY toInteger(number) WITH paID, title, venue,  conID, jouID, year,COLLECT(authors) AS authors, citations, saRank, futureRank, pageRank, doi RETURN  title, authors, conID, jouID, year, venue, citations,  saRank, futureRank, doi, pageRank order by saRank desc ")
